...
bool AudioBuffer::loadFromFile(const std::string& filePath) {
    ...
    if (!sndFile) {
        VG_ERROR("Could not open audio: " + std::string(sf_strerror(sndFile)));
    }
    ...
    if(fileInfo.frames < 1 || fileInfo.frames > (sf_count_t)(INT_MAX / sizeof(short)) / numChannels) {
        VG_ERROR("Bad sample count in " + filePath);
        sf_close(sndFile);
        return false;
    }
    ...
    if (numFrames < 1) {
        free(membuf);
        sf_close(sndFile);
        VG_ERROR("Failed to read samples in " + filePath);
        return false;
    }
    ...
    if (numChannels == 1) {
        ...
        VG_ERROR("Number of channels not implemented: " + std::to_string(numChannels));
        sf_close(sndFile);
        return false;
    }
    ...
    err = alGetError();
    if(err != AL_NO_ERROR) {
        VG_ERROR("OpenAL Error: " + std::string(alGetString(err)));
